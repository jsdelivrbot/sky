                ...
                @foreach($items as $item )
                    <div class="col-md-3 col-sm-6 col-xs-12">
                        <div class="event">
                            ...
                            <p>{{date('Y-m-d',$item->launch_date)}}</p>
                            <p>{{date('h:i A',$item->time_from)}} - {{date('h:i A',$item->time_to)}}</p>
                            <p>{{$item->address}}, {{$item->city}}, {{$item->country}}</p>
                            <a class="btn nwbtn2" href="{{asset("assets/")}}/#squarespaceModal-2" data-toggle="modal">Details</a>
                        </div>
                    </div>
                @endforeach
            </div>
